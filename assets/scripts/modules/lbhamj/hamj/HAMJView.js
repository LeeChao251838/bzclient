...
cc.Class({
    ...
    initHander() {
        ...
        game.addHandler('push_lazhuang_ctc', function (data) {
            console.log("==>> push_lazhuang_ctc --> data: ", data);
            self.laZhuang();
        });

        game.addHandler('lazhuang_result_push_ctc', function (data) {
            console.log("==>> lazhuang_result_push_ctc --> data: ", data);
            self.laZhuangResult(data);
        });

        game.addHandler('can_hu_prompt_ctc', function (data) {
            console.log("==>> can_hu_prompt_ctc --> data: ", data);
        ...
        game.addHandler('tingpai', function (event) {
            ...
            let index = cc.fy.gameNetMgr.getSeatIndexByID(userId);
            let localIndex = cc.fy.gameNetMgr.getLocalIndex(index);
            self._seats[localIndex].setIsTing(true, data.isTianTing == true);
        });
    ...
    initView() {
        ...
        cc.fy.utils.addClickEvent(this._canCancelTing, this.node, 'HAMJView', 'cancelTingClick');
    ...
    onGameBegin() {
        ...
        if (cc.fy.hamjMsg.otherHasAction) {
            this.showOtherHasAction();
            cc.fy.hamjMsg.otherHasAction = false;
        }
