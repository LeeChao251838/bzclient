cc.Class({
    ...
    initHandlers: function () {
        ...
        // 拉庄
        cc.fy.gameMsg.addHandler("push_lazhuang", function (data) {
            cc.fy.gameNetMgr.dispatchEvent("push_lazhuang_ctc", data);
        });

        // 拉庄结果
        cc.fy.gameMsg.addHandler("lazhuang_result_push", function (data) {
            cc.fy.gameNetMgr.dispatchEvent("lazhuang_result_push_ctc", data);
        });

        // 检测架听胡牌
        cc.fy.gameMsg.addHandler("can_hu_prompt", function (data) {
            cc.fy.gameNetMgr.dispatchEvent("can_hu_prompt_ctc", data);
        });

        // 架听返回
        cc.fy.gameMsg.addHandler("IsTing", function (data) {
            self.tingData = data;
            var si = cc.fy.gameNetMgr.getSeatIndexByID(self.tingData.userId);
            if (si != -1) {
                cc.fy.gameNetMgr.seats[si].tingState = 1;
                cc.fy.gameNetMgr.seats[si].jiating = data.pai;
            }
        ...
        cc.fy.gameMsg.addHandler("show_ting_view", function (data) {
