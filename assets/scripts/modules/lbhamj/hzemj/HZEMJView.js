...
var GameDef = require("HZEMJGameDef");
cc.Class({
    ...
    onGameBegin() {
        ...
        if (cc.fy.hzemjMsg.otherHasAction) {
            this.showOtherHasAction();
            cc.fy.hzemjMsg.otherHasAction = false;
        }
    ...
    initMahjongs() {
        ...
        for (var j = lackingNum; j < this._myMJArr.length; ++j) {
            let sprMJ = this._myMJArr[j];
            let card = sprMJ.node.mjId;
            if (cc.fy.mahjongmgr.isBaida(sprMJ.node.mjId)) {//cc.fy.mahjongmgr.isBaida(sprMJ.node.mjId)
