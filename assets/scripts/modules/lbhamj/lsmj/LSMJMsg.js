cc.Class({
    ...
    initHandlers:function()
    {
        ...
        cc.fy.gameMsg.addHandler("game_sync_push", function(data){
            if(cc.fy.gameNetMgr.conf.type == cc.GAMETYPE.SZER){
                self.tiantingState = false;
                var seats = cc.fy.gameNetMgr.seats;
                for(var i = 0; i < seats.length; ++i){
                    var seat = seats[i];
                    var sd = data.seats[i];
                    if (sd.userid == cc.fy.userMgr.userId) {
                        if(sd.tianting == 1){
                            self.tiantingState = true;
                            seat.tianting = 1;
                            cc.fy.gameNetMgr.tiantingAction = true;
                        }
                    }
               }
        ...
        //涟水天听消息监听处理-----------------------------------
        cc.fy.gameMsg.addHandler("game_tian_ting_cnt",function(data){
            console.log("log---->>>>game_tian_ting_cnt");
            cc.fy.gameNetMgr.dispatchEvent("game_tian_ting_cnt_push");
        });

        cc.fy.gameMsg.addHandler("game_tian_ting_push",function(data){
            console.log("log---->>>>>>game_tian_ting_push");
            for(var i = 0; i < cc.fy.gameNetMgr.seats.length; ++i){
                if(data != null && cc.fy.gameNetMgr.seats[i].userid == data){
                    self.tiantingState = true;
                    cc.fy.gameNetMgr.seats[i].tianting = 1;
                }
            }
            cc.fy.gameNetMgr.dispatchEvent("game_tian_ting_ctc");
        });

        cc.fy.gameMsg.addHandler("IsTianTing",function(data){
            var newdata = {
                userId:data.userId,
                pai:data.pai,
            ...
            self.tiantingState = false;
            if(data.tingstate == 0){
                cc.fy.gameNetMgr.dispatchEvent('game_guotingresult_ctc');
            }
            else{
                var si = cc.fy.gameNetMgr.getSeatIndexByID(data.userId);
                if(si != -1){
                    cc.fy.gameNetMgr.seats[si].tingState = 2;
                    cc.fy.gameNetMgr.seats[si].jiating = [];
                    cc.fy.gameNetMgr.seats[si].tiantingstate = data.tiantingstate;
                }
                // cc.fy.gameNetMgr.dispatchEvent('game_tingresult_ctc', newdata);
                if(data.userId == cc.fy.userMgr.userId) {
                    cc.fy.gameNetMgr.dispatchEvent("tingpai_ctc", newdata);
                }
                else{
                    cc.fy.gameNetMgr.dispatchEvent("otherTingPai_ctc", newdata);
                }
            }
        });
    },
