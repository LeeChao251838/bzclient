cc.Class({
    ...
    initView:function(){
        ...
        this.queYiMen.active = cc.fy.gameNetMgr.isDingQueing;
        ...
        if(cc.fy.gameNetMgr.gamestate == "dingque"){
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_dingque_notify',function(data){
            var seatIndex = cc.fy.gameNetMgr.getSeatIndexByID(data.detail);
        ...
        this.node.on('game_dingque_finish',function(){
            ...
            cc.fy.gameNetMgr.isDingQueing = false;
            self.initDingQue();
        });
    },
    ...
    showDingQueChoice:function(){
        ...
        var sd = cc.fy.gameNetMgr.getSelfData();
        var typeCounts = [0,0,0];
        for(var i = 0; i < sd.holds.length; ++i){
            var pai = sd.holds[i];
            var type = cc.fy.mahjongmgr.getMahjongType(pai);
            typeCounts[type]++;
        }
    ...
    initDingQue:function(){
        var arr = ["tong","tiao","wan"];
        var data = cc.fy.gameNetMgr.seats;
    ...
    onQueYiMenClicked:function(event){
        ...
        // cc.fy.gameNetMgr.dingque = type;
        // cc.fy.net.send("dingque",type);
